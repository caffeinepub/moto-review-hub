...
export default function MotorcycleReviews() {
  ...
  const filteredAndSortedReviews = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        (review) =>
          review.model.toLowerCase().includes(query) ||
          review.manufacturer.toLowerCase().includes(query)
      );
    }
      ...
      <div className="container mx-auto px-4">
        <div className="mb-12">
          ...
          <p className="text-xl text-muted-foreground">
            Expert reviews on motorcycles from riders who know them best
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredAndSortedReviews.map((review, index) => (
              <MotorcycleReviewCard key={index} review={review} index={reviews.indexOf(review)} />
            ))}
          </div>
        ) : (
          <div className="text-center py-20">
            <p className="text-muted-foreground text-lg">
              {searchQuery
                ? 'No motorcycle reviews found matching your search.'
                : 'No motorcycle reviews yet. Be the first to submit one!'}
            </p>
          </div>
        )}
      </div>
