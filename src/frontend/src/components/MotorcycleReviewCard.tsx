...
import { type MotorcycleReview } from '../backend';
import StarRating from './StarRating';
import { Calendar, User, Bike } from 'lucide-react';

interface MotorcycleReviewCardProps {
  review: MotorcycleReview;
  index: number;
}

export default function MotorcycleReviewCard({ review, index }: MotorcycleReviewCardProps) {
  ...
  return (
    ...
    >
      <div className="aspect-[4/3] overflow-hidden bg-muted flex items-center justify-center">
        <Bike size={80} className="text-moto-orange/30" />
      </div>
      <div className="p-6">
        ...
        <p className="text-sm font-semibold text-muted-foreground mb-3">{review.manufacturer}</p>
